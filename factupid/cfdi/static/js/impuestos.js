  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      document.querySelectorAll('.formulario-concepto').forEach(f => {
          ...
          if (tipoSelect && impuestoInput && tasaInput && factorInput) {
            ...
            const porcentaje = parseFloat(tasa.valor_maximo) || 0
            if (tasa.rango === "Fijo") {
                porcentajeInput.readOnly = true;
                porcentajeInput.classList.add("bg-light");
                porcentajeInput.title = "Este impuesto tiene un porcentaje fijo";
                porcentajeInput.removeAttribute("max");
            } else if (tasa.rango === "Rango") {
                porcentajeInput.readOnly = false;
                porcentajeInput.setAttribute("max", porcentaje.toString());
                porcentajeInput.title = `Máximo permitido: ${porcentaje}`;
            }
  ...
  function agregarImpuesto(formularioConcepto, tasaData) {
    ...
    const importe = Math.floor((base * (porcentaje / 100)) * 100) / 100;
    ...
    const porcentajeInput = nuevoForm.querySelector('[name$="porcentaje"]');
    if (porcentajeInput) {
        const valor = parseFloat(tasaData.porcentaje) || 0;
        porcentajeInput.value = valor;

        if (tasaData.rango === "Fijo") {
            // 🔒 Impuesto fijo: no editable
            porcentajeInput.readOnly = true;
            porcentajeInput.classList.add("bg-light");
            porcentajeInput.title = "Este impuesto tiene un porcentaje fijo";
        } else if (tasaData.rango === "Rango") {
            // 🔓 Impuesto con rango: editable pero con límite
            porcentajeInput.readOnly = false;
            porcentajeInput.setAttribute("max", valor.toString());
            porcentajeInput.title = `Máximo permitido: ${valor}`;
        }
    }
          ...
          item.addEventListener('click', function (e) {
              ...
              tasas.forEach(tasa => {
                  ...
                  <li>
                      ...
                      data-porcentaje="${tasa.valor_maximo}"
                      data-rango="${tasa.rango}">
  ...
  function actualizarIconoYTipo(formulario, tipoActual, esDoble) {
      ...
      const tipoFactor = formulario.querySelector('[name$="c_TipoFactor"]')?.value;

      // Si el tipo factor es Exento y el impuesto es IVA o IEPS
      if ((cod === '002' || cod === '003') && tipoFactor === 'Exento') {
        textoImpuesto += ' Exento';
      } else {
